...
if (!process.env.NODE_ENV) {
  process.env.NODE_ENV = JSON.parse(config.vconsole.env.NODE_ENV);
}
...
const webpackConfig = require('./webpack.vconsole.conf');
const utils = require('./utils');

// 默认调试服务器端口
let port = process.env.PORT || config.vconsole.port;

// 启动调试服务器时是否自动打开浏览器，默认为 false
let autoOpenBrowser = !!config.vconsole.autoOpenBrowser;
...
// 指定需要代理的请求列表
let proxyTable = config.vconsole.proxyTable;
...
// 纯静态资源服务
let staticPath = path.posix.join(config.vconsole.assetsPublicPath, config.vconsole.assetsSubDirectory);
